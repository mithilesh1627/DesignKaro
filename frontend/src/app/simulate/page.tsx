...
export default function SimulatePage() {
  ...
  const runSimulation = async () => {
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/api/v1/simulations/run", {
        method: "POST",
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-rose-400 mb-1">
              ...
              <span>PHASES 11–13: SYSTEM SIMULATOR &amp; CHAOS RESILIENCE</span>
            </div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">
              Traffic Load &amp; Failure Injection Simulator
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Simulate dynamic traffic spikes up to 100k+ QPS, inject real-time production faults, and inspect blast radius.
            </p>
          </div>
        </div>

        {/* Control Console */}
        <div className="p-6 rounded-2xl bg-surface-900/80 border border-slate-800 mb-8 space-y-6 shadow-xl">
          <div className="flex items-center gap-2 text-xs font-mono font-bold text-sky-400 border-b border-slate-800 pb-3">
            <Sliders className="h-4 w-4" />
            <span>SIMULATION CONTROLS &amp; CHAOS CONFIGURATION</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-1.5">
                <span className="text-slate-400">Baseline QPS:</span>
                <span className="text-sky-400 font-bold">{baseQps.toLocaleString()} QPS</span>
              </div>
              <input
                type="range"
                min={1000}
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-1.5">
                <span className="text-slate-400">Peak Surge QPS:</span>
                <span className="text-amber-400 font-bold">{peakQps.toLocaleString()} QPS</span>
              </div>
              <input
                type="range"
                min={10000}
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-sky-500 font-mono"
              >
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white focus:outline-none focus:border-sky-500 font-mono"
              >
                <option value="db1">PostgreSQL Primary (db1)</option>
                <option value="cache1">Redis Cluster (cache1)</option>
                <option value="s1">Application Service (s1)</option>
                <option value="gw1">API Gateway (gw1)</option>
              </select>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-2 border-t border-slate-800/80">
            <button
              ...
              disabled={isRunning}
              className="px-6 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-400 disabled:bg-slate-800 text-slate-950 font-bold text-xs font-mono flex items-center gap-2 transition-all shadow-lg shadow-rose-500/20"
            >
              {isRunning ? (
              ...
              <span>{isRunning ? "Simulating Traffic..." : "Start Simulation & Inject Chaos"}</span>
            </button>
          </div>
        </div>
        ...
        {simResult && activeTick && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              <div className="p-4 rounded-xl bg-surface-900/60 border border-slate-800">
              ...
              <div className="p-4 rounded-xl bg-surface-900/60 border border-slate-800">
                <div className="text-[10px] font-mono text-slate-500 uppercase">Live Ingress QPS</div>
              ...
              <div className="p-4 rounded-xl bg-surface-900/60 border border-slate-800">
                <div className="text-[10px] font-mono text-slate-500 uppercase">p99 Tail Latency</div>
                <div
              ...
              <div className="p-4 rounded-xl bg-surface-900/60 border border-slate-800">
                <div className="text-[10px] font-mono text-slate-500 uppercase">Dropped Requests</div>
                <div
                  className={`text-xl font-mono font-bold mt-1 ${
              ...
              <div className="p-4 rounded-xl bg-surface-900/60 border border-slate-800">
                ...
                <div
                  className={`text-sm font-mono font-bold mt-1.5 uppercase ${
            ...
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center gap-4">
              <button
                ...
                className="px-3 py-1 rounded bg-slate-800 hover:bg-slate-700 text-xs font-mono text-white"
              >
              ...
              <span className="text-xs font-mono text-slate-400 shrink-0">
                Tick {currentTickIndex + 1} of {simResult.ticks.length}
              </span>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {activeTick.node_metrics.map((n) => (
                  <div
                    key={n.node_id}
                    className={`p-4 rounded-xl border bg-surface-900/70 transition-all ${
                      n.status === "CRASHED"
                    ...
                    <div className="space-y-2 text-xs font-mono">
                      <div>
                        <div className="flex justify-between text-[11px] mb-1">
                          ...
                          <span className="text-slate-200">{n.cpu_percent}%</span>
                        </div>
                      ...
                      <div className="flex justify-between text-[11px] pt-1">
                        ...
                        <span className="text-slate-200">{n.queue_depth.toLocaleString()}</span>
                      </div>

                      <div className="flex justify-between text-[11px]">
                        <span className="text-slate-400">Tail Latency:</span>
                        <span className="text-sky-300">{n.latency_p99_ms.toFixed(1)}ms</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="p-6 rounded-2xl bg-surface-900/60 border border-slate-800 space-y-4">
              <div className="text-xs font-mono font-bold text-rose-400 flex items-center gap-1.5">
                <AlertTriangle className="h-4 w-4" />
                <span>POST-MORTEM &amp; INCIDENT ANALYSIS</span>
              </div>
              ...
              {simResult.incident_rca && (
                <div className="p-4 rounded-xl bg-rose-500/5 border border-rose-500/20 text-xs text-rose-300 leading-relaxed font-mono">
                  {simResult.incident_rca}
                </div>
              )}
